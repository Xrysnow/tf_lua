...
void TFE_ContextOptionsSetConfig(
...
// thread.
TFE_ContextDevicePlacementPolicy TFE_ContextGetDevicePlacementPolicy(TFE_Context* ctx);
...
void TFE_ContextSetServerDef(TFE_Context* ctx,
                                                   int keep_alive_secs,
                                                   const void* proto,
                                                   size_t proto_len,
                                                   TF_Status* status);
...
int TFE_TensorHandleNumDims(TFE_TensorHandle* h,
                                                  TF_Status* status);
int64_t TFE_TensorHandleNumElements(TFE_TensorHandle* h,
                                                          TF_Status* status);
// This function will block till the operation that produces `h` has completed.
int64_t TFE_TensorHandleDim(TFE_TensorHandle* h,
                                                  int dim_index,
                                                  TF_Status* status);
...
const char* TFE_TensorHandleDeviceName(
    TFE_TensorHandle* h, TF_Status* status);

// Returns the name of the device in whose memory `h` resides.
//
// This function will block till the operation that produces `h` has completed.
const char* TFE_TensorHandleBackingDeviceName(
    TFE_TensorHandle* h, TF_Status* status);
...
TFE_TensorHandle* TFE_TensorHandleCopySharingTensor(
    TFE_TensorHandle* h, TF_Status* status);
...
TFE_TensorDebugInfo* TFE_TensorHandleTensorDebugInfo(
    TFE_TensorHandle* h, TF_Status* status);

// Deletes `debug_info`.
void TFE_DeleteTensorDebugInfo(
...
int TFE_TensorDebugInfoOnDeviceNumDims(
    TFE_TensorDebugInfo* debug_info);
...
// The return value was current at the time of TFE_TensorDebugInfo creation.
int64_t TFE_TensorDebugInfoOnDeviceDim(
...
TFE_Op* TFE_NewOp(TFE_Context* ctx,
...
const char* TFE_OpGetName(const TFE_Op* op,
                                                TF_Status* status);
TFE_Context* TFE_OpGetContext(const TFE_Op* op,
                                                    TF_Status* status);
...
void TFE_OpAddInput(TFE_Op* op, TFE_TensorHandle* input,
                                          TF_Status* status);
...
int TFE_OpGetFlatInputCount(const TFE_Op* op,
                                                  TF_Status* status);
...
TF_AttrType TFE_OpNameGetAttrType(
    TFE_Context* ctx, const char* op_or_function_name, const char* attr_name,
    unsigned char* is_list, TF_Status* status);

void TFE_OpSetAttrString(TFE_Op* op,
                                               const char* attr_name,
                                               const void* value,
                                               size_t length);
void TFE_OpSetAttrInt(TFE_Op* op, const char* attr_name,
                                            int64_t value);
void TFE_OpSetAttrFloat(TFE_Op* op, const char* attr_name,
                                              float value);
...
void TFE_OpSetAttrFunction(TFE_Op* op,
                                                 const char* attr_name,
                                                 const TFE_Op* value);
...
void TFE_OpSetAttrTensor(TFE_Op* op,
                                               const char* attr_name,
                                               TF_Tensor* tensor,
                                               TF_Status* status);

void TFE_OpSetAttrStringList(TFE_Op* op,
                                                   const char* attr_name,
...
void TFE_OpSetAttrIntList(TFE_Op* op,
                                                ...
                                                int num_values);
void TFE_OpSetAttrFloatList(TFE_Op* op,
                                                  const char* attr_name,
                                                  const float* values,
                                                  int num_values);
void TFE_OpSetAttrBoolList(TFE_Op* op,
                                                 const char* attr_name,
                                                 const unsigned char* values,
                                                 int num_values);
void TFE_OpSetAttrTypeList(TFE_Op* op,
                                                 const char* attr_name,
                                                 const TF_DataType* values,
                                                 int num_values);
void TFE_OpSetAttrShapeList(
    TFE_Op* op, const char* attr_name, const int64_t** dims,
    const int* num_dims, int num_values, TF_Status* out_status);
void TFE_OpSetAttrFunctionList(TFE_Op* op,
                                                     const char* attr_name,
                                                     const TFE_Op** value,
                                                     int num_values);
...
void TFE_ContextAddFunction(TFE_Context* ctx,
...
void TFE_ContextRemoveFunction(TFE_Context* ctx,
                                                     const char* name,
                                                     TF_Status* status);

// Checks whether a function is registered under `name`.
unsigned char TFE_ContextHasFunction(TFE_Context* ctx,
                                                    const char* name);
...
// done.
void TFE_ContextExportRunMetadata(TFE_Context* ctx,
                                                        TF_Buffer* buf,
                                                        TF_Status* status);
