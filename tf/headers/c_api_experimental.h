...
void TF_EnableXLACompilation(TF_SessionOptions* options,
...
unsigned char TF_SetXlaEnableLazyCompilation(
    unsigned char enable);
...
void TF_SetXlaConstantFoldingDisabled(
...
// otherwise.
TF_Buffer* TF_CreateRunOptions(
    unsigned char enable_full_trace);

// Returns the graph content in a human-readable format, with length set in
// `len`. The format is subject to change in the future.
// The returned string is heap-allocated, and caller should call free() on it.
const char* TF_GraphDebugString(TF_Graph* graph,
                                                      size_t* len);
...
char* TF_FunctionDebugString(TF_Function* func,
                                                   size_t* len);
...
TF_Tensor* TF_DequeueNamedTensor(TF_Session* session,
                                                       int tensor_id,
                                                       TF_Status* status);
...
// TODO(hongm): Add support for `timeout_ms`.
void TF_EnqueueNamedTensor(TF_Session* session,
                                                 int tensor_id,
                                                 TF_Tensor* tensor,
                                                 TF_Status* status);
...
void TF_MakeInternalErrorStatus(TF_Status* status,
                                                      const char* errMsg);
...
TF_CheckpointReader* TF_NewCheckpointReader(
    const char* filename, TF_Status* status);
void TF_DeleteCheckpointReader(
    TF_CheckpointReader* reader);
int TF_CheckpointReaderHasTensor(
    TF_CheckpointReader* reader, const char* name);
// Get the variable name at the given index
const char* TF_CheckpointReaderGetVariable(
...
void TF_AttrBuilderSetType(TF_AttrBuilder* builder,
                                                 const char* attr_name,
                                                 TF_DataType value);
void TF_AttrBuilderSetTypeList(TF_AttrBuilder* builder,
                                                     const char* attr_name,
...
// run on device_type.
void TF_AttrBuilderCheckCanRunOnDevice(
...
const char* TF_GetNumberAttrForOpListInput(
...
int TF_OpIsStateful(const char* op_type,
                                          TF_Status* status);
...
// in tests only.)
int TF_PickUnusedPortOrDie(void);

// Fast path method that makes constructing a single scalar tensor require less
// overhead and copies.
TFE_TensorHandle* TFE_NewTensorHandleFromScalar(
...
void TFE_EnableCollectiveOps(TFE_Context* ctx,
                                                   const void* proto,
                                                   size_t proto_len,
                                                   TF_Status* status);
...
void TFE_AbortCollectiveOps(TFE_Context* ctx,
                                                  TF_Status* status);
...
void TFE_CollectiveOpsCheckPeerHealth(
...
//
TF_ShapeAndTypeList* TF_NewShapeAndTypeList(
    int num_shapes);
void TF_ShapeAndTypeListSetShape(
    TF_ShapeAndTypeList* shape_list, int index, const int64_t* dims,
    int num_dims);
void TF_ShapeAndTypeListSetUnknownShape(
    TF_ShapeAndTypeList* shape_list, int index);
void TF_ShapeAndTypeListSetDtype(
    TF_ShapeAndTypeList* shape_list, int index, TF_DataType dtype);
void TF_DeleteShapeAndTypeList(
    TF_ShapeAndTypeList* shape_list);
void TF_DeleteShapeAndTypeListArray(
